...
def Evaluate():
    ...
    for k in range(N): 
        ...
        _, p = sim.simxGetObjectPosition(clientID, CoppeliaSim_payload, -1, sim.simx_opmode_oneshot_wait)
        _, v, w = sim.simxGetObjectVelocity(clientID,CoppeliaSim_payload, sim.simx_opmode_oneshot_wait)
        _, q = sim.simxGetObjectQuaternion(clientID, CoppeliaSim_payload, -1, sim.simx_opmode_oneshot_wait)
        # Euler angles [alpha beta gamma]
        _, Euler =sim.simxGetObjectOrientation(clientID, CoppeliaSim_payload, -1, sim.simx_opmode_oneshot_wait) 
        pl = np.array([p]).T  #pi
        vl = np.array([v]).T  #vi
        ql = np.array([q]).T  #qi
        wl = np.array([w]).T  #wi
        Eulerl = np.array([Euler]).T
        # pl           = output_l['pl_new']
        # vl           = output_l['vl_new']
        # ql           = output_l['ql_new']
        # wl           = output_l['wl_new']
        ...
        for i in range(nq):
            ...
            UAVs = [UAV1, UAV2, UAV3]
            _, p_UAV = sim.simxGetObjectPosition(clientID, UAVs[i], -1, sim.simx_opmode_oneshot_wait)
            _, v_UAV, w_UAV = sim.simxGetObjectVelocity(clientID,UAVs[i], sim.simx_opmode_oneshot_wait)
            _, q_UAV = sim.simxGetObjectQuaternion(clientID, UAVs[i], -1, sim.simx_opmode_oneshot_wait)
            # Euler angles [alpha beta gamma]
            _, Euler_UAV =sim.simxGetObjectOrientation(clientID, UAVs[i], -1, sim.simx_opmode_oneshot_wait) 
            pi = np.array([p_UAV]).T  #pi
            vi = np.array([v_UAV]).T  #vi
            qi = np.array([q_UAV]).T  #qi
            wi = np.array([w_UAV]).T  #wi
            Euleri = np.array([Euler_UAV]).T
            Pi[i]        = pi     #'pi',3,1 position of CoM in the world frame {I}
            Vi[i]        = vi     #'vi',3,1 velocity of CoM in {I}
            Qi[i]        = qi     #'qi',4,1 quaternion of quadrotor
            Wi[i]        = wi     #'wi',3,1 angular velocity in the quadrotor's body frame {Bi}
            EULERi[i]    = Euleri #Euler_i_new = np.array([[gamma_i, theta_i, psi_i]]).T
            # pi           = output_i['pi_new']
            # vi           = output_i['vi_new']
            # qi           = output_i['qi_new']
            # wi           = output_i['wi_new']
...
if __name__ == '__main__':
    ...
    for k_train in range(1):
        ...
        ret, UAV1 = sim.simxGetObjectHandle(clientID, 'qd_1', sim.simx_opmode_blocking)
        ret, UAV2 = sim.simxGetObjectHandle(clientID, 'qd_2', sim.simx_opmode_blocking)
        ret, UAV3 = sim.simxGetObjectHandle(clientID, 'qd_3', sim.simx_opmode_blocking)
        ret, CoppeliaSim_payload = sim.simxGetObjectHandle(clientID, 'Cylinder', sim.simx_opmode_blocking)
        for k in range(600): 
            # payload's state
            xl           = np.vstack((pl, vl, ql, wl))
            # quadrotors' current states 
            ...
            for i in range(nq):
                ...
                # CoppeliaSim 
                # fi,Mi# 4-by-1 vector
                CoppeliaSim_fi = ui[0][0]
                CoppeliaSim_Mi_x = ui[1][0]
                CoppeliaSim_Mi_y = ui[2][0]
                CoppeliaSim_Mi_z = ui[3][0]
                print("----------------------------------------")
                print(CoppeliaSim_fi,CoppeliaSim_Mi_x,CoppeliaSim_Mi_y,CoppeliaSim_Mi_z)
                print("----------------------------------------")
                # CoppeliaSim_fi = np.round(CoppeliaSim_fi, 1)
                # CoppeliaSim_Mi_x = np.round(CoppeliaSim_Mi_x, 1)
                # CoppeliaSim_Mi_y = np.round(CoppeliaSim_Mi_y, 1)
                # CoppeliaSim_Mi_z = np.round(CoppeliaSim_Mi_z, 1)
                # thrust = np.array([0.0, 0.0, 0.0], dtype=np.float32)         ###可能需要X.X
                # torque = np.array([0.0,0.0,0.0],dtype=np.float32)
                # # i-UAV ID 0 1 2 
                thrust = np.array([0.0, 0.0, CoppeliaSim_fi], dtype=np.float32)
                torque = np.array([CoppeliaSim_Mi_x,CoppeliaSim_Mi_y,CoppeliaSim_Mi_z],dtype=np.float32)
                torque_packed = struct.pack('f' * len(torque), *torque)
                thrust_packed = struct.pack('f' * len(thrust), *thrust)
                sim.simxSetStringSignal(clientID,'start_'+str(i+1),'s', sim.simx_opmode_oneshot_wait)
                sim.simxSetStringSignal(clientID,'torque_'+str(i+1), torque_packed, sim.simx_opmode_oneshot_wait)
                sim.simxSetStringSignal(clientID,'thrust_'+str(i+1), thrust_packed, sim.simx_opmode_oneshot_wait)
                
                # sim.simxSetFloatSignal(clientID,'torque_x_'+str(i+1), CoppeliaSim_Mi_x, sim.simx_opmode_oneshot_wait)
                # sim.simxSetFloatSignal(clientID,'torque_y_'+str(i+1), CoppeliaSim_Mi_y, sim.simx_opmode_oneshot_wait)
                # sim.simxSetFloatSignal(clientID,'torque_z_'+str(i+1), CoppeliaSim_Mi_z, sim.simx_opmode_oneshot_wait)
                # sim.simxSetFloatSignal(clientID,'thrust_'+str(i+1), CoppeliaSim_fi, sim.simx_opmode_oneshot_wait)
                # CoppeliaSim.fmcommand(i, thrust, torque)
                # CoppeliaSim_output_i = CoppeliaSim.getqdstate(i)
                UAVs = [UAV1, UAV2, UAV3]
                _, p_UAV = sim.simxGetObjectPosition(clientID, UAVs[i], -1, sim.simx_opmode_oneshot_wait)
                _, v_UAV, w_UAV = sim.simxGetObjectVelocity(clientID,UAVs[i], sim.simx_opmode_oneshot_wait)
                _, q_UAV = sim.simxGetObjectQuaternion(clientID, UAVs[i], -1, sim.simx_opmode_oneshot_wait)
